...
for i in range(0,3):
	for j in (1,2,4,8,16,32,64,128,256,512):
		cmd="./dineroIV -l1-usize 8k -l1-ubsize 16 -l1-uassoc " +str(j)+ " -l1-uwback a -l1-uwalloc a -l1-uccc -informat d < "+ans[i]+" | grep rate >> "+files[i]
		os.system(cmd)
...
plt.figure()
plt.plot(b,a1)
plt.xlabel("associative size")
plt.ylabel("miss rate")
plt.savefig("q4-cc1.png")
plt.figure()
file1.close()
...
plt.figure()
plt.plot(b,a2)
plt.xlabel("assosiative size")
plt.ylabel("miss rate")
plt.savefig("q4-spice.png")
plt.figure()
file2.close()
